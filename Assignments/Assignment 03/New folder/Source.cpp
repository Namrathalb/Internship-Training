...
void htmltoexcel::intial(string fileName)
{
    ...
    while (line != "</html>") {
        ...
        getline(myfile, line);
        if (line == " ") {
            cout << "File cannot open! Doesn't Exist in Directory.";
            logfile(__LINE__, "intial()", "File Doesn't Exist in Directory");
            break;
        }
        if (data.size() == 6) {
...
int main()
{
    ...
    if (b == 1) {
        ...
        logfile(__LINE__, " main() ", "File Doesn't Exist in Directory");
    }
    else {
        cout << "File Exists!!!" << "\n";
